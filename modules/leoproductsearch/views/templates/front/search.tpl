...
{block name='content'}
	...
	class="page-heading {if !isset($instant_search) || (isset($instant_search) && !$instant_search)} product-listing{/if}">
		{l s='Rasta' mod='leoproductsearch'}&nbsp;
		{if $nbProducts > 0}
			<span class="lighter">
		...
		{else}
			<span class="heading-counter">
				{if $nbProducts == 1}{l s='%d rezultatų.' sprintf=[$nbProducts|intval] mod='leoproductsearch'}{else}{l s='%d rezultatų.' sprintf=[$nbProducts|intval] mod='leoproductsearch'}{/if}
			</span>
		{/if}
	</h1>
	...
	{if !$nbProducts}
		<p class="alert alert-warning">
			{if isset($search_query) && $search_query}
				{l s='Pagal jūsų paiešką rezultatų nerasta' mod='leoproductsearch'}&nbsp;"{if isset($search_query)}{$search_query|escape:'html':'UTF-8'}{/if}"
			{elseif isset($search_tag) && $search_tag}
				{l s='Pagal jūsų paiešką rezultatų nerasta' mod='leoproductsearch'}&nbsp;"{$search_tag|escape:'html':'UTF-8'}"
			{else}
