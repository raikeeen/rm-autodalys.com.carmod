...
{block name='content'}
	<section id="main">
		<div id="mywishlist">
			<h2>{l s='Įsimintų prekių sarašas' mod='leofeature'}</h2>
			<div class="new-wishlist">
				<div class="form-group">
					<label for="wishlist_name">{l s='Pavadinimas' mod='leofeature'}</label>
					<input type="text" class="form-control" id="wishlist_name" placeholder="{l s='Įveskite naujo pageidavimų sąrašo pavadinimą' mod='leofeature'}">
				</div>
				...
				<div class="list-wishlist">
					<table class="table table-striped">
					  <thead class="wishlist-table-head">
						<tr>
						  <th>{l s='Pavadinimas' mod='leofeature'}</th>
						  <th>{l s='Kiekis' mod='leofeature'}</th>
						  <th>{l s='Peržiūrėta' mod='leofeature'}</th>
						  <th class="wishlist-datecreate-head">{l s='Sukurtas' mod='leofeature'}</th>
						  <th>{l s='Tiesioginė nuoroda' mod='leofeature'}</th>
						  <th>{l s='Numatytas' mod='leofeature'}</th>
						...
						{if $wishlists}
							{foreach from=$wishlists item=wishlists_item name=for_wishlists}
								<tr>					 
									...
									<td><a class="view-wishlist" data-token="{$wishlists_item.token}" target="_blank" href="{$view_wishlist_url}{if $leo_is_rewrite_active}?{else}&{/if}token={$wishlists_item.token}" title="{l s='Vaizdas' mod='leofeature'}">{l s='Vaizdas' mod='leofeature'}</a></td>
									<td>
									...
									<td><a class="delete-wishlist" data-id-wishlist="{$wishlists_item.id_wishlist}" href="javascript:void(0)" title="{l s='Ištrinti' mod='leofeature'}"><i class="material-icons">&#xE872;</i></a></td>
								</tr>
							{/foreach}
						{/if}
					  </tbody>
			...
			<div class="send-wishlist">
				<a class="leo-send-wishlist-button btn btn-info" href="javascript:void(0)" title="{l s='Siųsti šį pageidavimų sąrašą' mod='leofeature'}">
					<i class="material-icons">&#xE163;</i>
					{l s='Išsiųsti sarašą' mod='leofeature'}
				</a>
			</div>
			<section id="products">
			...
			<ul class="footer_links">
				<li class="pull-xs-left"><a class="btn btn-outline" href="{$link->getPageLink('my-account', true)|escape:'html'}"><i class="material-icons">&#xE317;</i>{l s='Grįžti į paskyrą' mod='leofeature'}</a></li>
