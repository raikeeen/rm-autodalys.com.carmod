...
$aTip = Array(
	...
	12 => 'Этот html DOM элемент используется в скрипте AJAX добавления товаров в корзину /'.CM_DIR.'/templates/default/includes.php как PHP константа: CmsCartID<br>Так же для обновления содержимого выпадающей корзинки можно добавить соответствующий js триггер в /'.CM_DIR.'/tocms/MyCMS-AfterCartAjax.js - где "MyCMS" это название вашей CMS. Он будет вызван после AJAX обновления корзины. Оставьте поле пустым чтобы не использовать AJAX а просто перезагружать страницу браузера.',
	13 => 'Будет показано "В наличии" или "Нет в наличии"',
	14 => '<ul>
			<li>Шаблоны модуля располагаются в папке /carparts/templates/.. Не следует редактировать шаблон "<b>default</b>" так как он постоянно <b>перезаписывается</b> системой авто-обновлений. Что бы создать собственный шаблон - скопируйте и переименуйте (по FTP) шаблон "default" а затем переключитесь на него в админ-части модуля, в основных настройках.</li>
			<li>Если вы будете переносить папку модуля на другой хост или заново загружать из бэкапа - то используйте <b>Двоичный</b> (бинарный) режим FTP - не используйте ASCII режим, так как скрипты закодированы в двоичном и не будут выполняться выдавая ошибку "Fatal error: The encoded file ... is corrupt"</li>
		</ul>',
	...
	16 => 'CSS код цвета (6 символов) основных элементов шаблона используемый в /carparts/templates/default/includes.php',
