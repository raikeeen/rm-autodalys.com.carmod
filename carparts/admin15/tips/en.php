...
$aTip = Array(
	...
	12 => 'This html DOM element is used in the AJAX script for adding products to the cart in /'.CM_DIR.'/templates/default/includes.php as PHP constant: CmsCartID<br>You can also add the corresponding js trigger in /'.CM_DIR.'/tocms/MyCMS-AfterCartAjax.js - where the “MyCMS” is the name of your CMS, to update the contents of the dropdown basket. It will be called after the AJAX cart update. Leave the field blank to not use AJAX but just reload the browser page',
	13 => '"Available" or "Out of stock" will be displayed',
	14 => '<ul>
			<li>The plugin templates are located in the /carparts/templates/.. folder. Do not edit the "<b>default</b>" template as it is constantly <b>overwritten</b> by the auto-update system. To create your own template - copy and rename the template (by FTP) "default" and then switch to it in the admin-side of the plugin, in the basic settings.</li>
			<li>If you transfer the plugin folder to another host or re-upload from the backup, then use the FTP <b>Binary</b> mode - do not use the ASCII mode, as the scripts are encoded in binary and will not execute producing an error "Fatal error: The encoded file ... is corrupt"</li>
			</ul>',
	...
	16 => 'CSS color code (6 characters) of the main elements of the template used in /carparts/templates/default/includes.php',
